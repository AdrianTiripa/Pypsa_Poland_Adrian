...
def solve(n: pypsa.Network, cfg: dict) -> tuple[pypsa.Network, str, str]:
    ...
    # result = n.optimize(solver_name=name, solver_options=options) ========== This is where the nuclear problem was coming from, the min capacity was not set
    ...
    result = n.optimize(
        ...
        extra_functionality=lambda n, snapshots: fix_total_capacity_by_carrier(n, snapshots, cfg),
    )
    # =========== nuclear fix
...
def run_pipeline(cfg: dict) -> Path:
    ...
    # #  PV FIX ========================================================================= Not sure if it works, might be ignorable
