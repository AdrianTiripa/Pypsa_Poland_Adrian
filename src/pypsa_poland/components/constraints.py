...
def fix_total_capacity_by_carrier(n: pypsa.Network, snapshots, cfg: dict) -> None:
    ...
    gas_min = float(cap_cfg.get("natural_gas_min_mw", 20_200))
    nuclear_min = float(cap_cfg.get("nuclear_min_mw", 19_843))
    ...
    if len(gas_gens) > 0:
        ...
        m.add_constraints(expr >= gas_min, name="total_gas_capacity_min")
    else:
    ...
    if len(nuclear_gens) > 0:
        ...
        m.add_constraints(expr >= nuclear_min, name="total_nuclear_capacity_min")
    else:
