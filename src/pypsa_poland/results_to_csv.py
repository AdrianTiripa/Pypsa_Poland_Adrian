# src/pypsa_poland/results_to_csv.py
#
# Structured CSV export of key results from one or more pypsa-poland run folders.
#
# Reads the raw PyPSA output CSVs (generators, links, storage_units, loads, and
# their time-series counterparts) and writes a numbered set of analysis-ready
# summary CSVs into a "csv_results/" sub-folder of each run directory.
#
# Outputs (numbered for easy downstream ingestion):
#   01  capacity by region and carrier (generators)
#   02  capacity by carrier (system total)
#   03  annual generation by region and carrier [MWh]
#   04  annual generation by carrier [MWh]
#   05  generator capacity factors
#   06  annual load by region [MWh]
#   07  total annual system load [MWh]
#   08  interregional transmission flow summary
#   09  net imports by region [MWh]
#   10  transmission utilisation summary
#   11  storage capacity by carrier
#   12  storage capacity by asset
#   13  storage dispatch summary (charge, discharge, mean/max SOC)
#   14  sector coupling capacity summary
#   15  hydrogen sector summary
#   16  heat sector summary
#   17  transport sector summary
#   18  VRES curtailment detail
#   19  VRES curtailment by carrier
#   20  dominant power flow direction
#
# Usage:
#   python -m pypsa_poland.results_to_csv --run_dir  <path>
#   python -m pypsa_poland.results_to_csv --runs_root <folder_of_runs>
...
# Carriers classified as variable renewable energy sources for curtailment analysis.
